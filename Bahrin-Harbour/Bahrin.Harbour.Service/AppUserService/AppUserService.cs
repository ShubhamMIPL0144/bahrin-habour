...
namespace Bahrin.Harbour.Service.AppUserService
{
    public class AppUserService : IAppUserService
    {
        ...
        private readonly string imageFolderName = Constants.AppUserImages;
        ...
        public async Task<StatusModel> AddAppUserAsync(AppUserViewModel appUser)
        {
            ...
            {
                applicationUser.ImageName = await _image.SaveImageAsync(appUser.Image, imageFolderName);
                applicationUser.ImagePathfolder = imageFolderName;
            }
            ...
            try
            {
                ...
                if (user == null)
                {
                    var userModel = new AppUserViewModel
                    {
                        ...
                        Email = user.Email,
                        ImageLink = _image.GenerateImageUrl(user.ImagePathfolder, user.ImageName),
        ...
        public async Task<List<AppUserViewModel>> GetAllAppUsersAsync()
        {
            try
            {
                ...
                var userModel = Users.Select(x => new AppUserViewModel
                {
                    ...
                    Email = x.Email,
                    ImageLink = _image.GenerateImageUrl(x.ImagePathfolder, x.ImageName),
                    LastName = x.LastName,
                    IsActive = x.IsActive,
        ...
        public async Task<StatusModel> UpdateAppUserAsync(AppUserViewModel appUser)
        {
            try
            {
                ...
                if (existingUser != null)
                {
                    if (!string.IsNullOrEmpty(existingUser.ImageName))
                    {
                      await _image.UpdateImageAsync(appUser.Image, imageFolderName, existingUser.ImageName);
                    }
                    ...
                    if (appUser.Image != null)
                    {
                        existingUser.ImageName = await _image.SaveImageAsync(appUser.Image, imageFolderName);
                        existingUser.ImagePathfolder = imageFolderName;
                    }
        ...
        public async Task<StatusModel> HardDeleteAppUserAsync(string userId)
        {
            try
            {
                ...
                if (existingUser != null)
                {
                    if (!string.IsNullOrEmpty(existingUser.ImageName))
                    {
                       if(await Helper.Helper.FileExistsAsync(Path.Combine(imageFolderName, existingUser.ImageName)))
                        {
                            _image.DeleteImage(imageFolderName, existingUser.ImageName);
                        }
                    }
